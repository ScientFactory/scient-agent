...
export function githubRepositoryFromRemote(remote: string) {
  ...
  const urlMatch =
    /^(?:ssh:\/\/git@|https?:\/\/)github\.com\/([^/]+)\/(.+)$/i.exec(trimmed);
...
export function parseUpstreamState(value: unknown): UpstreamState {
  ...
  if (!UPDATE_MODES.has(state.updateMode)) {
    throw new Error(
      `${UPSTREAM_STATE_PATH} has unsupported updateMode ${state.updateMode}.`,
    );
  }
  for (const [key, commit] of [
    ...
    if (!/^[0-9a-f]{40}$/.test(commit)) {
      throw new Error(
        `${UPSTREAM_STATE_PATH} field ${key} must be a full lowercase commit SHA.`,
      );
    }
  }
  if (!/^\d{4}-\d{2}-\d{2}$/.test(state.reviewedAt)) {
    throw new Error(
      `${UPSTREAM_STATE_PATH} field reviewedAt must use YYYY-MM-DD.`,
    );
  }
  return state;
}
...
function requireString(value: Record<string, unknown>, key: string) {
  ...
  throw new Error(
    `${UPSTREAM_STATE_PATH} field ${key} must be a non-empty string.`,
  );
}

function assertGitHubRemote(
  label: string,
  remote: string,
...
async function main() {
  ...
  if (mode === "intake" && initialStatus) {
    throw new Error(
      "Run Scient agent intake verification from a clean worktree.",
    );
  }
  ...
  const state = parseUpstreamState(
    JSON.parse(
      readFileSync(path.join(process.cwd(), UPSTREAM_STATE_PATH), "utf8"),
    ),
  );
  ...
  const unreviewedCommits = Number(
    run("git", [
      "rev-list",
      "--count",
      `${state.reviewedThrough}..${upstreamTip}`,
    ]),
  );
    ...
    JSON.stringify(
      {
        ...
        sourceVersion: JSON.parse(
          readFileSync(
            path.join(process.cwd(), "packages/opencode/package.json"),
            "utf8",
          ),
        ).version,
